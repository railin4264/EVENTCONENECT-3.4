...
import { Search, MapPin, Calendar, Users, Sparkles, Zap, Target, Globe, Music, Palette, Settings } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          >
            ...
            <span className="text-cyan-400 font-medium">
              La plataforma social más avanzada para conectar con eventos y tribus.
            </span>
          </motion.p>
        </motion.div>
        ...
        >
          <h3 className="text-lg font-semibold text-white mb-6">Personaliza tu experiencia</h3>
        ...
        >
          <FeatureIcon icon={<Search className="w-6 h-6" />} label="Búsqueda Inteligente" delay={0} />
          <FeatureIcon icon={<Users className="w-6 h-6" />} label="Comunidades" delay={0.1} />
          <FeatureIcon icon={<MapPin className="w-6 h-6" />} label="Ubicación" delay={0.2} />
          <FeatureIcon icon={<Sparkles className="w-6 h-6" />} label="Experiencias" delay={0.3} />
        </motion.div>
        ...
        >
          <DynamicIllustration
            type="person"
            context={currentTheme.name.toLowerCase().includes('mañana') ? 'morning' : 
                   currentTheme.name.toLowerCase().includes('tarde') ? 'afternoon' :
                   currentTheme.name.toLowerCase().includes('atardecer') ? 'evening' : 'night'}
            size="large"
            interactive={true}
          />
        </motion.div>
        ...
        >
          <MorphingCard variant="breathing" className="h-full">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-gray-300">
                Descubre eventos antes de que los busques con nuestra IA avanzada
              </p>
            </CardContent>
          </MorphingCard>
          ...
          <MorphingCard variant="breathing" className="h-full">
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Conectividad Global</h3>
              <p className="text-gray-300">
        ...
        >
          <div className="flex items-center justify-center space-x-3">
            ...
            <span className="text-white/80">
              Tema actual: <span className="text-cyan-400 font-medium">{currentTheme.name}</span>
            </span>
            <span className="text-white/60">•</span>
            <span className="text-white/80">
              Estado de ánimo: <span className="text-cyan-400 font-medium">{currentTheme.mood}</span>
            </span>
          </div>
        </motion.div>
      </div>
