    ...
    >
      ...
      <div className='p-6'>
        ...
        <div className='mb-4'>
          ...
          <DescriptionWithClamp text={event.description} />
        </div>
          ...
          {event.tags && event.tags.length > 0 && (
            <div className='flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400'>
              ...
              <div className='flex flex-wrap gap-1'>
                ...
                {event.tags.length > 3 && (
                  <Badge size='sm' variant='outline'>+{event.tags.length - 3}</Badge>
                )}
              </div>
            </div>
          )}
...
// Local component: description with clamp and expand
const DescriptionWithClamp: React.FC<{ text: string }> = ({ text }) => {
  const [expanded, setExpanded] = useState(false);
  if (!text) return null;
  return (
    <div>
      <p className={cn('text-gray-600 dark:text-gray-400', expanded ? '' : 'line-clamp-2')}>
        {text}
      </p>
      {text.length > 140 && (
        <button
