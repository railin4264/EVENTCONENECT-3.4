...
import { Search, Clock, MapPin, Users, TrendingUp, Sparkles, Settings, Palette, Type, Layout, Eye, X, Music } from 'lucide-react';
...
const PredictiveSuggestions: React.FC<{ actions: PredictiveAction[] }> = ({ actions }) => {
...
const AdvancedCustomization: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const tabs = [
    ...
    { id: 'animations', label: 'Animaciones', icon: <Sparkles className="w-4 h-4" /> }
  ];
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">Personalización Avanzada</h2>
          <button
            onClick={onClose}
          ...
          >
            {activeTab === 'layout' && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-white mb-3">Diseño de Layout</h3>
                  ...
                  {['grid', 'list', 'masonry', 'timeline'].map((layout) => (
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 transition-all ${
                        preferences.find(p => p.category === 'layout')?.value === layout
                          ? 'border-blue-400 bg-blue-400/20'
                          : 'border-white/20 bg-white/5 hover:border-white/40'
                      }`}
                    >
                      <div className="text-white font-medium capitalize">{layout}</div>
                      <div className="text-white/60 text-sm">Vista en {layout}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {activeTab === 'colors' && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-white mb-3">Esquema de Colores</h3>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { id: 'auto', name: 'Automático', desc: 'Se adapta al contexto' },
                    { id: 'light', name: 'Claro', desc: 'Tema claro fijo' },
                    { id: 'dark', name: 'Oscuro', desc: 'Tema oscuro fijo' },
                    { id: 'custom', name: 'Personalizado', desc: 'Colores únicos' }
                  ].map((theme) => (
                    <button
                      key={theme.id}
                      onClick={() => updatePreference('colors', theme.id)}
                      className={`p-4 rounded-xl border-2 transition-all ${
                        preferences.find(p => p.category === 'colors')?.value === theme.id
                          ? 'border-blue-400 bg-blue-400/20'
                          : 'border-white/20 bg-white/5 hover:border-white/40'
                      }`}
                    >
            ...
            {activeTab === 'typography' && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-white mb-3">Tipografía</h3>
                <div className="space-y-3">
                  {[
                    { id: 'inter', name: 'Inter', desc: 'Moderno y legible' },
                    { id: 'roboto', name: 'Roboto', desc: 'Clásico y confiable' },
                    { id: 'poppins', name: 'Poppins', desc: 'Elegante y amigable' },
                    { id: 'mono', name: 'Monospace', desc: 'Técnico y preciso' }
                  ].map((font) => (
                    <button
                      key={font.id}
                      onClick={() => updatePreference('typography', font.id)}
                      className={`w-full p-4 rounded-xl border-2 transition-all text-left ${
                        preferences.find(p => p.category === 'typography')?.value === font.id
                          ? 'border-blue-400 bg-blue-400/20'
                          : 'border-white/20 bg-white/5 hover:border-white/40'
                      }`}
            ...
            {activeTab === 'density' && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-white mb-3">Densidad de Información</h3>
                <div className="space-y-3">
                  {[
                    { id: 'compact', name: 'Compacta', desc: 'Máxima información, mínimo espacio' },
                    { id: 'comfortable', name: 'Cómoda', desc: 'Balance entre espacio y contenido' },
                    { id: 'spacious', name: 'Espaciosa', desc: 'Máximo espacio, fácil lectura' }
                  ].map((density) => (
                    <button
                      key={density.id}
                      onClick={() => updatePreference('density', density.id)}
                      className={`w-full p-4 rounded-xl border-2 transition-all text-left ${
                        preferences.find(p => p.category === 'density')?.value === density.id
                          ? 'border-blue-400 bg-blue-400/20'
                          : 'border-white/20 bg-white/5 hover:border-white/40'
                      }`}
                    >
                      <div className="text-white font-medium">{density.name}</div>
                      <div className="text-white/60 text-sm">{density.desc}</div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {activeTab === 'animations' && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-white mb-3">Nivel de Animaciones</h3>
                <div className="space-y-3">
                  {[
                    { id: 'minimal', name: 'Mínimas', desc: 'Solo transiciones esenciales' },
                    { id: 'smooth', name: 'Suaves', desc: 'Animaciones fluidas y elegantes' },
                    { id: 'extensive', name: 'Extensivas', desc: 'Máximo movimiento y efectos' }
                  ].map((animation) => (
                    <button
                      key={animation.id}
                      onClick={() => updatePreference('animations', animation.id)}
                      className={`w-full p-4 rounded-xl border-2 transition-all text-left ${
                        preferences.find(p => p.category === 'animations')?.value === animation.id
                          ? 'border-blue-400 bg-blue-400/20'
                          : 'border-white/20 bg-white/5 hover:border-white/40'
                      }`}
                    >
                      <div className="text-white font-medium">{animation.name}</div>
                      <div className="text-white/60 text-sm">{animation.desc}</div>
                    </button>
                  ))}
                </div>
              </div>
...
export const PredictiveUI: React.FC<PredictiveUIProps> = ({ userId, className = '' }) => {
  ...
  useEffect(() => {
    const analyzeBehavior = () => {
      ...
      setPredictiveActions(actions.sort((a, b) => b.probability - a.probability));
    };
  ...
  useEffect(() => {
    ...
    const actions = ['music-events', 'tech-events', 'social-events', 'work-events'];
    const randomAction = () => {
  ...
  return (
    <div className={className}>
      ...
      <AnimatePresence>
        {isCustomizationOpen && (
          <AdvancedCustomization onClose={() => setIsCustomizationOpen(false)} />
        )}
      </AnimatePresence>
    </div>
  );
