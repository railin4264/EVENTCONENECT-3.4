      ...
      <Card variant="glass" className={cn('w-full', getCategoryBg())}>
        <CardContent className="p-4">
          <div className="flex items-start space-x-3">
            {/* Icon */}
            <div className={cn('w-10 h-10 rounded-lg flex items-center justify-center', getCategoryBg())}>
                ...
                >
                  {feature.id === 'font-size' && (
                    <div className="space-y-2">
                      <label className="text-sm text-gray-300">Tamaño de fuente:</label>
                      <div className="flex space-x-2">
                        {['small', 'medium', 'large', 'xlarge'].map((size) => (
                          <button
                            key={size}
                            onClick={() => onSettingsChange?.(feature.id, { size })}
                            className={cn(
                  ...
                  {feature.id === 'color-blind-mode' && (
                    <div className="space-y-2">
                      <label className="text-sm text-gray-300">Tipo de daltonismo:</label>
                      <select
                        onChange={(e) => onSettingsChange?.(feature.id, { type: e.target.value })}
                        className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white text-sm"
                      >
                        <option value="none">Ninguno</option>
                        <option value="protanopia">Protanopía (rojo-verde)</option>
                        <option value="deuteranopia">Deuteranopía (rojo-verde)</option>
...
const KeyboardNavigationGuide: React.FC<{
  ...
  const shortcuts = [
    ...
    { key: 'P', description: 'Ir a perfil', icon: <ArrowRight className="w-4 h-4" /> },
    { key: 'S', description: 'Buscar', icon: <ArrowRight className="w-4 h-4" /> },
    { key: 'A', description: 'Accesibilidad', icon: <ArrowRight className="w-4 h-4" /> }
  ];
          ...
          {shortcuts.map((shortcut, index) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-300">Tecla: {shortcut.key}</div>
              </div>
            </motion.div>
          ))}
        </div>
  ...
  return (
    <Card variant="glass" className={cn('w-full', className)}>
      ...
      <CardContent className="space-y-4">
        ...
        {/* ARIA Labels */}
        <div className="space-y-2">
          <h5 className="text-white font-medium">Etiquetas ARIA Implementadas:</h5>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {[
              'aria-label',
              'aria-describedby',
...
const ColorBlindSimulator: React.FC<{
  ...
  const colorBlindTypes = [
    ...
    { id: 'protanopia', name: 'Protanopía', description: 'Dificultad para distinguir rojo y verde' },
    { id: 'deuteranopia', name: 'Deuteranopía', description: 'Dificultad para distinguir rojo y verde' },
    { id: 'tritanopia', name: 'Tritanopía', description: 'Dificultad para distinguir azul y amarillo' }
  ];
    ...
    <Card variant="glass" className={cn('w-full', className)}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-3 bg-blue-400/10 border border-blue-400/20 rounded-lg">
          <h6 className="text-blue-400 font-medium mb-2">Consejos de Accesibilidad:</h6>
...
export const AccessibilitySystem: React.FC = () => {
  ...
  useEffect(() => {
    ...
    document.documentElement.classList.toggle('simplified-layout', settings.simplifiedLayout);
    document.documentElement.classList.toggle('reading-mode', settings.readingMode);
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-white/5 rounded-lg">
                  ...
                  <div className="text-sm text-gray-300">Características Activas</div>
                </div>
                <div className="text-center p-4 bg-white/5 rounded-lg">
                  <div className="text-2xl font-bold text-blue-400 mb-1">
                    {features.filter(f => f.category === 'visual' && f.enabled).length}
                  </div>
                  <div className="text-sm text-gray-300">Visual Activo</div>
                </div>
                <div className="text-center p-4 bg-white/5 rounded-lg">
                  <div className="text-2xl font-bold text-purple-400 mb-1">
                    {features.filter(f => f.category === 'audio' && f.enabled).length}
                  </div>
                  <div className="text-sm text-gray-300">Audio Activo</div>
                </div>
                <div className="text-center p-4 bg-white/5 rounded-lg">
                  <div className="text-2xl font-bold text-orange-400 mb-1">
                    {features.filter(f => f.category === 'navigation' && f.enabled).length}
                  </div>
