  ...
  const updateAnalytics = (newLocation: LocationData) => {
    ...
    setAnalytics(prev => ({
      ...
      averageSpeed: prev.averageSpeed > 0 ? 
        (prev.averageSpeed + (distance / 1000)) / 2 : distance / 1000,
    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600 mt-2">
          Tracking en tiempo real, geofencing automático y análisis de movimientos
        </p>
      </div>
        ...
        <Card variant="glass" className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="font-mono text-lg">±{Math.round(currentLocation.accuracy)}m</p>
            </div>
            <div className="text-center">
      ...
      <Card variant="glass" className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-2">
            {isTracking ? (
              <Button onClick={stopTracking} variant="outline" className="text-red-600 border-red-600">
                <WifiOff className="w-4 h-4 mr-2" />
                Detener
              </Button>
            ) : (
              <Button onClick={startTracking} variant="primary" disabled={permissionStatus !== 'granted'}>
        ...
        {isTracking && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-3">
            <div className="flex items-center space-x-2">
              ...
              <p className="text-green-700 text-sm">Tracking activo - Monitoreando ubicación en tiempo real</p>
            </div>
          </div>
        )}
      </Card>
      ...
      <Card variant="glass" className="p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center">
            ...
            <p className="font-semibold text-lg">{formatDistance(analytics.totalDistance)}</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-gray-600">Velocidad Promedio</p>
            <p className="font-semibold text-lg">{analytics.averageSpeed.toFixed(1)} km/h</p>
          </div>
          <div className="text-center">
            <p className="text-sm text-gray-600">Tiempo Activo</p>
            <p className="font-semibold text-lg">{formatTime(analytics.timeSpent)}</p>
          </div>
          <div className="text-center">
      ...
      <Card variant="glass" className="p-6">
        ...
        <div className="space-y-3">
          {geofences.map(geofence => (
            <div key={geofence.id} className="flex items-center justify-between p-3 bg-white/50 rounded-lg">
              <div>
                <h4 className="font-medium">{geofence.name}</h4>
                <p className="text-sm text-gray-600">
                  Radio: {formatDistance(geofence.radius)} • Eventos: {geofence.events.join(', ')}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${geofence.isActive ? 'bg-green-500' : 'bg-gray-400'}`}></div>
      ...
      <Card variant="glass" className="p-6">
        ...
        <div className="space-y-3 max-h-64 overflow-y-auto">
          {locationEvents.length === 0 ? (
            <p className="text-gray-500 text-center py-4">No hay eventos de ubicación aún</p>
          ) : (
            locationEvents.slice().reverse().map(event => (
              <div key={event.id} className="flex items-center space-x-3 p-3 bg-white/50 rounded-lg">
                <div className={`w-2 h-2 rounded-full ${
                  event.type === 'enter' ? 'bg-green-500' :
                  event.type === 'exit' ? 'bg-red-500' :
                  event.type === 'nearby' ? 'bg-yellow-500' : 'bg-blue-500'
                }`}></div>
      ...
      {locationHistory.length > 0 && (
        <Card variant="glass" className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <span className="text-sm text-gray-600">({locationHistory.length} puntos)</span>
          </div>
          
          <div className="max-h-48 overflow-y-auto">
            <div className="space-y-2">
              {locationHistory.slice().reverse().slice(0, 10).map((location, index) => (
                <div key={index} className="flex items-center space-x-3 p-2 bg-white/30 rounded">
