...
const usePushNotifications = () => {
  ...
  const sendSubscriptionToServer = async subscription => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notifications/push-token`,
        {
          method: 'POST',
          headers: {
  ...
  const removeSubscriptionFromServer = async subscription => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notifications/push-token/${encodeURIComponent(
  ...
  const syncPendingNotifications = async () => {
    try {
      ...
      if (pendingNotifications.length > 0) {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/notifications/sync`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${
  ...
  const sendNotificationMetric = async (action, data) => {
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/notifications/metrics`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${
