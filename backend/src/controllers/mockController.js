const { mockUsers, mockEvents, mockTribes, mockPosts } = require('../config/mockData');
...
class MockController {
  ...
  async getTribes(req, res) {
    try {
      ...
      // Filtrar por categoría
      if (category && category !== 'all') {
        filteredTribes = filteredTribes.filter(tribe => tribe.category === category);
      }
      ...
      if (search) {
        filteredTribes = filteredTribes.filter(tribe =>
          ...
          tribe.tags.some(tag => tag.toLowerCase().includes(search.toLowerCase()))
        );
      }
