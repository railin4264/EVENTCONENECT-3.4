...
class DemandAnalyticsService {
  ...
  async calculateDemandMetrics(location, radius) {
    ...
    const categoryDemand = await this.calculateCategoryDemand(
      usersInArea,
      recentEvents
    );
    ...
    return {
      ...
      competition,
      pricing,
      marketSaturation: this.calculateMarketSaturation(
        usersInArea.length,
        recentEvents.length
      ),
    };
  ...
  calculateCategoryDemand(users, events) {
    ...
    categories.forEach(category => {
      ...
      const recentEvents = events.filter(event => event.category === category);
      ...
      const demandSupplyRatio =
        recentEvents.length > 0
          ? interestedUsers / recentEvents.length
          : interestedUsers;
      ...
      demand[category] = {
        ...
        opportunityScore,
        averageAttendance:
          recentEvents.length > 0 ? totalAttendees / recentEvents.length : 0,
  ...
  calculateOpportunityScore(interestedUsers, eventCount, totalAttendees) {
    ...
    const attendanceScore =
      totalAttendees > 0 ? Math.min(totalAttendees / 100, 10) * 10 : 50; // Historial de asistencia
      ...
      demographics.ageGroups[group] = {
        ...
        percentage:
          demographics.totalUsers > 0
            ? (demographics.ageGroups[group] / demographics.totalUsers) * 100
            : 0,
      };
    });
  ...
  analyzeCompetition(events) {
    ...
    Object.keys(eventsByCategory).forEach(category => {
      ...
      const totalAttendees = categoryEvents.reduce(
        (sum, event) => sum + (event.attendees?.length || 0),
        0
      );
      ...
      competitionMetrics[category] = {
        ...
        averageAttendees:
          categoryEvents.length > 0
            ? totalAttendees / categoryEvents.length
            : 0,
        marketShare:
          events.length > 0 ? (categoryEvents.length / events.length) * 100 : 0,
        competitionIntensity:
          this.calculateCompetitionIntensity(categoryEvents),
      };
    });
  ...
  calculateCompetitionIntensity(events) {
    ...
    const recentEvents = events.filter(
      event => now - new Date(event.startDate) < 30 * 24 * 60 * 60 * 1000
    ).length;
  ...
  getOptimalTiming(category) {
    ...
    return (
      timingMap[category] || 'Evalúa horarios según tu audiencia específica'
    );
  }
  ...
  calculateCategoryTrends(categoryTrends) {
    ...
    Object.entries(categoryTrends).forEach(([category, data]) => {
      ...
      const firstHalfAvg =
        firstHalf.reduce((sum, item) => sum + item.attendees, 0) /
        firstHalf.length;
      const secondHalfAvg =
        secondHalf.reduce((sum, item) => sum + item.attendees, 0) /
        secondHalf.length;
  ...
  calculateSimpleGrowth(data) {
    ...
    return (
      ((recent[1].attendees - recent[0].attendees) / recent[0].attendees) * 100
    );
  }
  ...
  async generateEventSuggestions(userId, location) {
    try {
      const user = await User.findById(userId).select(
        'interests preferences stats location'
      );
      ...
      // Análisis de demanda local
      const demandAnalysis = await this.analyzeLocalDemand(
      ...
      return {
        ...
        data: {
          ...
          recommendationReason:
            'Basado en tus intereses, experiencia como organizador y demanda local',
        },
      };
    } catch (error) {
  ...
  async generatePersonalizedSuggestions(user, demandData) {
      ...
      if (categoryData && categoryData.opportunityScore > 60) {
        suggestions.push({
          ...
          marketData: {
            ...
            suggestedPrice: this.suggestPrice(
              interest,
              categoryData,
              demandData.analysis.pricing
            ),
          },
    ...
    return suggestions.sort((a, b) => b.confidence - a.confidence).slice(0, 5); // Top 5 sugerencias
  }
  ...
  generateExperienceBasedSuggestions(user, demandData) {
    ...
    if (user.stats.averageRating > 4.0) {
      const highOpportunityCategories = Object.entries(
        demandData.analysis.categoryDemand
      )
