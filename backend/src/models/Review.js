...
const reviewSchema = new mongoose.Schema({
  ...
  targetOrganizer: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: function() { return this.reviewType === 'organizer'; }
  },
  
  targetUser: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: function() { return this.reviewType === 'user'; }
  },
  ...
  verificationMethod: {
    type: String,
    enum: ['attendance_confirmed', 'manual_verification', 'photo_proof', 'automated'],
...
reviewSchema.methods.markAsHelpful = function(userId) {
  const existingHelpful = this.helpful.find(h => h.user.toString() === userId.toString());
...
reviewSchema.statics.getAverageRating = async function(targetId, reviewType) {
  const query = {};
  query[`target${reviewType.charAt(0).toUpperCase() + reviewType.slice(1)}`] = targetId;
  ...
  if (data.ratings.length > 0) {
    ...
    for (const key in firstRating) {
      if (typeof firstRating[key] === 'number') {
        const values = data.ratings.map(r => r[key]).filter(v => v !== undefined);
        breakdown[key] = values.reduce((sum, val) => sum + val, 0) / values.length;
      }
    }
  }
...
reviewSchema.statics.getRatingDistribution = async function(targetId, reviewType) {
  const query = {};
  query[`target${reviewType.charAt(0).toUpperCase() + reviewType.slice(1)}`] = targetId;
  query['moderation.status'] = 'approved';
...
reviewSchema.statics.canUserReview = async function(userId, targetId, reviewType) {
  ...
  if (reviewType === 'event') {
    ...
    const attended = event.attendees.some(a => 
      a.userId.toString() === userId.toString() && a.status === 'confirmed'
    );
    
    if (!attended) {
      return { canReview: false, reason: 'Solo puedes reseñar eventos a los que asististe' };
    }
    
    // Check if event has already happened
    if (event.endDate > new Date()) {
      return { canReview: false, reason: 'Solo puedes reseñar eventos que ya hayan terminado' };
    }
  }
...
reviewSchema.post('save', async function(doc) {
  ...
  try {
    ...
    const ratingData = await Model.getAverageRating(
      doc[`target${doc.reviewType.charAt(0).toUpperCase() + doc.reviewType.slice(1)}`],
      doc.reviewType
    );
