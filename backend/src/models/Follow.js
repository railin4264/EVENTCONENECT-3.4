...
followSchema.statics.getSuggestedFollows = async function (
  userId,
  options = {}
) {
  ...
  try {
    ...
    const User = mongoose.model('User');
    const currentUser =
      await User.findById(userId).select('interests location');
...
followSchema.statics.followUser = async function (
  ...
  const { source = 'manual', context = {}, notifications = {} } = options;
...
followSchema.statics.updateFollowCounts = async function (
  followerId,
  followingId,
  action
) {
...
// ===== POST-SAVE HOOKS =====
followSchema.post('save', async doc => {
