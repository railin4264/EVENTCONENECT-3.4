...
badgeSchema.methods.checkUnlockCriteria = function (user, context) {
  switch (this.unlockCondition) {
    case 'achievement':
      ...
      const { achievementId } = this.unlockCriteria;
    ...
    case 'milestone':
      ...
      const { metric } = this.unlockCriteria;
      const { target } = this.unlockCriteria;
      ...
      return {
        ...
        reason:
          currentValue >= target
            ? null
            : `Need ${target - currentValue} more ${metric}`,
      };
    ...
    case 'event':
      ...
      return {
        ...
        reason: attendedEvents.includes(eventId)
          ? null
    ...
    case 'time_based':
      ...
      const { timeMetric } = this.unlockCriteria;
      const { timeTarget } = this.unlockCriteria;

      if (timeMetric === 'account_age') {
        const accountAge =
          (Date.now() - user.createdAt) / (1000 * 60 * 60 * 24); // días
        return {
          canEarn: accountAge >= timeTarget,
          reason:
            accountAge >= timeTarget
              ? null
              : `Account must be ${timeTarget} days old`,
        };
      }
...
badgeSchema.methods.evaluateSpecialCriteria = function (user, context) {
  ...
  if (criteria.type === 'combo') {
    ...
    const hasCombo = requiredActions.every(action =>
...
badgeSchema.methods.calculateRarityScore = function () {
  ...
  if (
    this.seasonalInfo?.isSeasonalAndActive &&
    new Date() > this.seasonalInfo.endDate
  ) {
    score += 15;
  }
