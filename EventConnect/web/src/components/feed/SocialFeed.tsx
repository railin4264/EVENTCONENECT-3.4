...
const SocialFeed = ({ 
  ...
  const { posts, isLoading, error, createPost, likePost, unlikePost } = usePosts();
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              @{post.author.username} • {formatDistanceToNow(new Date(post.createdAt), { 
                addSuffix: true, 
                locale: es 
              })}
            </p>
        ...
        <div className="mb-4">
          {post.media.images.length === 1 ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              {post.media.images.slice(0, 4).map((image: string, index: number) => (
