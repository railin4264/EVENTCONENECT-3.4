...
const SEOHead = ({ 
  ...
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://eventconnect.com';
  
  // Valores por defecto
  const defaultTitle = 'EventConnect - Descubre Eventos y Tribus Increíbles';
  const defaultDescription = 'Plataforma líder para descubrir eventos únicos y conectar con tribus cerca de ti. Encuentra eventos, únete a comunidades y vive experiencias inolvidables.';
  const defaultKeywords = [
    'eventos', 'tribus', 'comunidades', 'conexiones sociales', 'experiencias',
    'networking', 'actividades', 'grupos', 'eventos cercanos', 'social media',
    'eventos en vivo', 'meetups', 'conferencias', 'workshops', 'fiestas',
    'deportes', 'arte', 'música', 'tecnología', 'negocios', 'educación'
  ];
  ...
  return (
    <Head>
      ...
      {/* RSS Feed */}
      <link rel="alternate" type="application/rss+xml" title="EventConnect RSS Feed" href="/feed.xml" />
      
      {/* Hreflang para internacionalización */}
      <link rel="alternate" hrefLang="es" href={finalUrl} />
      <link rel="alternate" hrefLang="en" href={finalUrl.replace('/es/', '/en/')} />
