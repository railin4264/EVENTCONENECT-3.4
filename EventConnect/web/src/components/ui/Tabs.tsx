...
export const Tabs: React.FC<TabsProps> = ({
  ...
  const getVariantStyles = () => {
    switch (variant) {
      case 'default':
        return {
          container: 'bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-1',
      ...
      case 'glass':
        return {
          container: 'backdrop-blur-xl bg-white/10 border border-white/20 rounded-xl p-1',
      ...
      case 'gradient':
        return {
          container: 'bg-gradient-to-r from-cyan-500/20 to-purple-600/20 border border-cyan-400/30 rounded-xl p-1',
          tab: 'text-gray-300 hover:text-white hover:bg-white/10 rounded-lg',
          active: 'text-white bg-white/20',
  ...
  const renderTabButton = (tab: TabItem) => {
    ...
    return (
      <button
        ...
        disabled={isDisabled}
        ref={(el) => {
          if (el && isActive && variant !== 'pills' && variant !== 'underline') {
            updateIndicator(el);
          }
        }}
        className={cn(
