...
const eventSchema = new mongoose.Schema({
  ...
  category: {
    ...
    enum: [
      ...
      'networking', 'workshop', 'conference', 'party', 'meetup', 'concert',
      'festival', 'exhibition', 'seminar', 'webinar', 'competition', 'charity'
    ]
  },
  subcategory: {
    type: String,
...
eventSchema.methods.addToWaitlist = function(userId) {
  const existingWaitlist = this.waitlist.find(w => w.user.toString() === userId.toString());
  if (!existingWaitlist) {
...
// Static methods
eventSchema.statics.findNearby = function(coordinates, maxDistance = 10000, limit = 20) {
  return this.find({
    location: {
      $near: {
        $geometry: {
