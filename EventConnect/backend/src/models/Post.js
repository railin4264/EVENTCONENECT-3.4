...
const postSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['text', 'image', 'video', 'link', 'event', 'tribe', 'poll', 'article'],
    default: 'text'
  },
  title: {
    type: String,
  ...
  poll: {
    question: {
      type: String,
      required: true,
      maxlength: [200, 'La pregunta de la encuesta no puede exceder 200 caracteres']
    },
    options: [{
      text: {
        type: String,
...
postSchema.virtual('isLiked').get(function() {
  return (userId) => this.social.likes.some(like => 
    like.user.toString() === userId.toString()
  );
});

postSchema.virtual('isSaved').get(function() {
  return (userId) => this.social.saves.some(save => 
...
postSchema.virtual('isPollActive').get(function() {
  ...
  return this.poll.isActive && (!this.poll.endsAt || this.poll.endsAt > new Date());
});
...
postSchema.methods.toggleLike = function(userId) {
  const likeIndex = this.social.likes.findIndex(like => 
    like.user.toString() === userId.toString()
  );
...
postSchema.methods.toggleSave = function(userId) {
  const saveIndex = this.social.saves.findIndex(save => 
    save.user.toString() === userId.toString()
  );
...
postSchema.statics.findNearby = function(coordinates, maxDistance = 10000, limit = 20) {
  return this.find({
    location: {
      $near: {
        $geometry: {
