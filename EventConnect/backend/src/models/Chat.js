...
const messageSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['text', 'image', 'video', 'audio', 'file', 'location', 'event', 'tribe'],
    default: 'text'
  },
  media: {
    url: String,
...
chatSchema.methods.pinMessage = function(messageId, userId) {
  ...
  if (message) {
    const existingPin = this.pinnedMessages.find(p => 
      p.message.toString() === messageId.toString()
    );
...
chatSchema.pre('save', function(next) {
  ...
  if (this.type === 'private' && this.participants.length > 2) {
    return next(new Error('Los chats privados solo pueden tener 2 participantes'));
  }
