...
notificationSchema.virtual('canSend').get(function () {
  return (
    !this.isExpired && (!this.isScheduled || this.scheduledFor <= new Date())
  );
});
...
notificationSchema.methods.updateDeliveryStatus = function (
  ...
  if (this.delivery[channel]) {
    ...
    if (status === 'sent') {
      ...
      this.delivery[channel].failureReason =
        additionalData.reason || 'Unknown error';
    }
  }
...
notificationSchema.pre('save', function (next) {
  ...
  if (!this.category) {
    if (this.type.startsWith('event_')) {
      ...
      this.type.startsWith('post_') ||
      this.type.includes('follower') ||
      this.type.includes('mention')
    ) {
      ...
      this.type.includes('badge') ||
      this.type.includes('achievement')
    ) {
      this.category = 'badge';
  ...
  if (!this.priority) {
    if (this.type.includes('urgent') || this.type.includes('security')) {
      ...
      this.type.includes('reminder') ||
      this.type.includes('starting_soon')
    ) {
      this.priority = 'high';
    } else if (
      this.type.includes('welcome') ||
      this.type.includes('achievement')
    ) {
