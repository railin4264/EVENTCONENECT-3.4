...
scheduledNotificationSchema.methods.markAsFailed = function (error) {
  this.status =
    this.executionAttempts >= this.maxExecutionAttempts ? 'failed' : 'pending';
  this.lastExecutionAttempt = new Date();
  ...
  switch (this.recurrence.pattern) {
    ...
    case 'custom':
      ...
      if (this.recurrence.daysOfWeek && this.recurrence.daysOfWeek.length > 0) {
        ...
        const nextDay = this.recurrence.daysOfWeek.find(
          day => day > currentDay
        );
        if (nextDay !== undefined) {
          ...
          nextTime.setDate(
            nextTime.getDate() +
              (7 - currentDay + this.recurrence.daysOfWeek[0])
          );
        }
      }
      break;
...
scheduledNotificationSchema.methods.shouldExecuteNow = function () {
  ...
  if (this.conditions.timeWindow) {
    ...
    if (
      currentTime < this.conditions.timeWindow.start ||
...
// Métodos estáticos
scheduledNotificationSchema.statics.findReadyToExecute = function (
  limit = 100
) {
  const now = new Date();
  return this.find({
...
scheduledNotificationSchema.statics.findByUser = function (userId, limit = 50) {
  return this.find({ userId }).sort({ scheduledTime: 1 }).limit(limit);
};

scheduledNotificationSchema.statics.findPendingByUser = function (
  userId,
  limit = 50
) {
  return this.find({
...
scheduledNotificationSchema.pre('save', function (next) {
  ...
  if (this.recurrence.enabled) {
    if (
      ...
      this.scheduledTime > this.recurrence.endDate
    ) {
      this.status = 'expired';
    }
