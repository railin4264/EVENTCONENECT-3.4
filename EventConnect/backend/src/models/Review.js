...
const reviewSchema = new mongoose.Schema({
  ...
  categories: [{
    category: {
      type: String,
      enum: [
        'organization', 'communication', 'punctuality', 'venue', 'content',
        'atmosphere', 'value', 'accessibility', 'safety', 'overall'
      ],
      required: true
    },
    rating: {
  ...
  flags: [{
    ...
    reason: {
      type: String,
      enum: [
        'inappropriate', 'spam', 'fake', 'harassment', 'offensive',
...
reviewSchema.virtual('isHelpful').get(function() {
  return (userId) => this.helpful.some(h => h.user.toString() === userId.toString());
});

reviewSchema.virtual('isNotHelpful').get(function() {
  return (userId) => this.notHelpful.some(h => h.user.toString() === userId.toString());
});

reviewSchema.virtual('isFlagged').get(function() {
  return (userId) => this.flags.some(f => f.user.toString() === userId.toString());
});

// Methods
reviewSchema.methods.markHelpful = async function(userId) {
  // Remove from not helpful if present
  this.notHelpful = this.notHelpful.filter(h => h.user.toString() !== userId.toString());
...
reviewSchema.methods.addReply = async function(authorId, content, isOfficial = false) {
  this.replies.push({
    author: authorId,
    content,
    isOfficial
...
reviewSchema.methods.verifyReview = async function(verifiedBy, method = 'manual') {
...
reviewSchema.methods.moderateReview = async function(moderatedBy, status, reason = '', notes = '') {
  this.status = status;
...
reviewSchema.statics.findByEvent = function(eventId, options = {}) {
  const { page = 1, limit = 20, sort = 'createdAt', order = 'desc', status = 'approved' } = options;
...
reviewSchema.statics.getHostStats = async function(hostId) {
  ...
  Object.keys(categoryRatings).forEach(category => {
    categoryAverages[category] = categoryRatings[category].total / categoryRatings[category].count;
  });
