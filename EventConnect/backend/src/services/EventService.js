...
class EventService {
  ...
  async getPersonalizedRecommendations(userId, limit = 10) {
    try {
      const user = await User.findById(userId).select('interests location following');
      if (!user) {
      ...
      throw new AppError(`Error al obtener recomendaciones: ${error.message}`, 500);
    }
  }
  ...
  async getEventAnalytics(eventId, hostId) {
    try {
      ...
      if (event.host.toString() !== hostId) {
        throw new AppError('No autorizado para ver analytics de este evento', 403);
      }
      ...
      await redis.set(`event:analytics:${eventId}`, JSON.stringify(analytics), 3600);
  ...
  async processEventCreation(eventData, hostId) {
    try {
      ...
      if (!validationResult.isValid) {
        throw new AppError(`Datos del evento inválidos: ${validationResult.errors.join(', ')}`, 400);
      }
      ...
      if (hostEventCount >= 10) {
        throw new AppError('Has alcanzado el límite de eventos activos (10)', 400);
      }
  ...
  async getNearbyEvents(location, radius = 50, filters = {}) {
    try {
      ...
      throw new AppError(`Error al obtener eventos cercanos: ${error.message}`, 500);
    }
  }
  ...
  calculateDemographics(attendees) {
    ...
    attendees.forEach(attendee => {
      ...
      // Gender
      if (attendee.gender) {
        demographics.gender[attendee.gender] = (demographics.gender[attendee.gender] || 0) + 1;
      }

      // Interests
      if (attendee.interests) {
        attendee.interests.forEach(interest => {
          demographics.topInterests[interest] = (demographics.topInterests[interest] || 0) + 1;
        });
      }
    });
