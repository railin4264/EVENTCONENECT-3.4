...
const createSpeedLimit = (windowMs = 15 * 60 * 1000, delayAfter = 50, delayMs = 100) => {
  return slowDown({
    windowMs,
    delayAfter,
    delayMs,
...
const authRateLimit = createRateLimit(15 * 60 * 1000, 5, 'Demasiados intentos de autenticación. Intente de nuevo en 15 minutos');
const apiRateLimit = createRateLimit(15 * 60 * 1000, 100, 'Demasiadas solicitudes a la API. Intente de nuevo en 15 minutos');
const uploadRateLimit = createRateLimit(15 * 60 * 1000, 10, 'Demasiadas subidas de archivos. Intente de nuevo en 15 minutos');
const searchRateLimit = createRateLimit(1 * 60 * 1000, 30, 'Demasiadas búsquedas. Intente de nuevo en 1 minuto');
...
const validateMethod = (allowedMethods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH']) => {
  return (req, res, next) => {
...
const requestFrequencyMonitor = () => {
  ...
  return (req, res, next) => {
    ...
    // Check frequency
    if (validRequests.length > 60) { // More than 60 requests per minute
