...
const sort = (req, res, next) => {
  ...
  const allowedSortFields = [
    'createdAt', 'updatedAt', 'name', 'title', 'date', 'popularity', 
    'rating', 'price', 'capacity', 'attendeesCount', 'likesCount',
    'views', 'lastActivity', 'totalMembers', 'totalEvents'
  ];
...
const locationFilter = (req, res, next) => {
  const { lat, lng, radius = 10, locationField = 'location.coordinates' } = req.query;
    ...
    if (req.pagination && Array.isArray(data)) {
      ...
      const response = {
        ...
        pagination: {
          ...
          pages: Math.ceil((req.totalCount || data.length) / req.pagination.limit)
        }
      };
